...
`meme-template-json-compiler` 将已审核并上传的模板图转化为可复用的 Gallery v2 模板 JSON。它独立分析图片的玩法、主体关系和可编辑内容，建立槽位、推荐项、用户提示词与运行约束，支持首次编译、批量处理和 JSON 局部返修。

## 输入与输出

输入为 Approved Template Image v2 envelope，包含批准状态和图片的 URL、SHA-256、宽高及格式。源身份由独立运行上下文提供，仅用于 key 注册表解析。
...
通过 `$meme-template-json-compiler` 调用，提供已批准图片的 envelope；局部返修同时提供上一版 JSON 和修改要求。完整工作流程见 [SKILL.md](SKILL.md)。

1. 校验输入，独立查看已批准图片。
2. 分析模板价值、玩法机制、主体关系与可编辑内容。
3. 编写槽位、文案、Prompt Template 和运行语义。
4. 解析 key，校验草稿并完成绑定草稿摘要的自复核。
5. 编译正式 JSON，写入交付目录并更新生产索引。
6. 任务涉及工作台时，校验实际采集的读回数据。
...
- key 由注册表解析；批量任务按 1–100 项分片，每项独立处理失败。
...
| `scripts/compiler.py` | 编译、校验、返修与交付函数 |
...
| 怎样确定身份、交付与当前版本 | [key-registry.md](references/key-registry.md)、[portable-delivery.md](references/portable-delivery.md)、[返修与读回校验.md](references/返修与读回校验.md) |

## 运行环境

Python 3.10+，macOS 或 Linux。依赖见 `requirements.txt`；文件写入需要支持硬链接的文件系统。编译器提供 Python 函数接口，图片理解由执行 Skill 的视觉 Agent 完成。
