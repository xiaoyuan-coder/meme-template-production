...
visualContract 是固定模板实现层，保存媒介、画风、质感、光线、边缘处理及玩法必需的空间关系。Prompt Template 已开放的颜色、形状、物件、包装内容、背景或动作不再进入本字段。`editableFactRouting.forbiddenRuntimeTerms` 记录要排除的当前值、旧值和同义表达，编译门禁逐条检查残留。
