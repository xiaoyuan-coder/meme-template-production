...
- 注册表必须返回 `EXISTING_SAME_SOURCE`。key、cover、referenceImage、imageUrl 保持原值；缺少可靠上一版时暂停该项的 JSON-only 返修，说明缺失入口。
...
- `content`：从该入口实际数据提取的正式字段。字段集合见 `machine-contract.json` 的 `deliveryReadback.surfaceFields`；详情与导出比对完整正式 JSON，列表与编辑预览比对对应投影。工作台包装字段先剥离，正式字段保留原值；正式 JSON 未提供的可选字段继续保持缺省。
