...
工作台默认展示每个稳定 key 最新修改后、已通过校验并完成正式交付的数据。返修交付完成后，列表、详情、槽位编辑预览和 JSON 导出都采用同一份当前交付；旧版本保留在明确标注的历史入口。

- 当前版本以同一模板修订链的生产记录与正式交付事实确定。索引 `generatedAt` 是整份索引的更新时间，文件修改时间和扫描顺序也不证明某个 key 更新；跨批次的局部 revision 数字不能直接比较。存在竞争来源且无法确认先后时，将该 key 标为版本冲突待核对。
- 新修改尚在分析、校验失败、写入失败或等待换图批准时，显示“有新修改待交付”及对应状态。需要展示上一版时，明确标注“上一版已交付”，避免将其标为最新修改结果。
- 每个新 revision 使用独立交付位置，沿用 create-once 写入并保留历史。先完成正式 JSON 与索引写入，再由本地数据台切换当前引用；同一视图中的标题、描述、槽位、推荐项、Prompt、运行语义和图片引用共同来自该 revision。
- JSON-only 修订继续复用已批准图片 URL，数据新鲜度通过正式 JSON 与修订记录核对。图片 URL 未变化不能作为跳过刷新或读回的依据。

`write_production_index` 成功表示便携产物已写入。任务要求进入模板数据看板时，每次初次交付和返修交付都需由本地数据台按其授权入口完成真实读回：
