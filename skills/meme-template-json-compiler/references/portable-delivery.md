...
单批支持 1–100 项，每项独立保存状态和恢复动作。注册表不可用、key 冲突、JSON 拒绝、OSS 失败或写文件冲突只暂停当前项。
...
<runtime-root>/
  ...
  sidecars/<itemId>/                  # analysis、JSON review、registry 证据和状态
  receipts/<itemId>/oss-receipt.json
