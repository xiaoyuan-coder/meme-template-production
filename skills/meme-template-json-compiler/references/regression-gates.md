...
- `editableFacts`：声明 Prompt 或槽位拥有的开放事实、整个 Runtime 禁止残留的旧值，以及必须闭合的全部 target。检查范围同时包含 `targetInstances.role/region` 和 `visualContract`。
- `tagging`：保留图片打标器的 `hiddenTags` 和 `keywords`，门禁重新合并后与 `metadata.tags` 按顺序精确比对。
- `surfaceAssertions`：可选的跨表面断言。每项指定 `promptTemplate`、`runtimeSemantics`、`runtimeSemantics.targetInstances` 或 `runtimeSemantics.visualContract`，并列出该表面必须出现和禁止出现的词。用它固定媒介归属、空间位置和历史旧值，补足自然语言语义无法自动推断的部分。

case 缺失、槽位集合不等、完整对象被拆成修饰词、开放事实被 Runtime 写死、联动 target 不全、跨表面断言不成立或 tag 合并漂移，均使回归失败。
