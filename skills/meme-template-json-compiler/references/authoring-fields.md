...
3. `coreUserDecisions`：用户会主动选择且会形成明显新版本的内容；每项决策一对一映射正式槽位。
...
普通餐食、背景小物、陪衬贴纸、泛化配饰、轻微颜色和渲染参数通常保持固定；它们明确承载当前玩法时可进入候选。正式槽位优先选择 2–4 个高价值编辑轴；八轴覆盖评审只得到一个核心用户决策时允许单槽。超过四个候选时，将低频文字路由为 `free_editable`，把支持性细节保持固定，或合并同一语义轴的控件；五个及以上槽位不能进入人工审核。
...
发现问题先修改草稿再重新复核；最新草稿 SHA 的全部检查通过后直接编译并交付。自复核不调用生图 API，也不创建人工审核点。
