...
- 自由编辑模式：用户直接编辑完整 Prompt Template。这段自然语言覆盖全部允许改动的画面内容，用户可修改已有事实或补充新的可编辑内容。

两种模式共享一套属性归属：Prompt/slot 管理可编辑内容，Runtime 管理输入到目标及其依赖部件的映射，visualContract 管理固定媒介、画风、质感、光线与模板必需的空间关系。一项可视属性只由一层持有，以 `editableFactRouting` 作为可追踪的边界证据。
...
召回轮按完整视觉对象建立候选，再将颜色、形状、边缘、材质和图案记为该对象的可变属性。例如“蓝色粗边爱心”是一个外框对象，用户可在同一控制中改颜色、边缘和形状；只抽出“蓝色粗边”会让爱心形状被另一层继续锁定。多个元素共同承担一种装饰职能、用户会用一句话整组替换且后端能统一绑定时，将它们合并为“周围装饰”等组合对象。

槽位数量是两轮判断的结果。1–4 个玩法槽位是常见范围，第五槽需要独立高价值证据。背景在每个模板中都进入候选召回，依据背景类型、用户价值、模型可控性和机制相容性决定是否开槽。背景默认值、推荐项与传图能力按单模板评估，不占用预留席位。

背景召回同时区分独立场景层、封闭容器内部底纹和画布外围留白。完整徽章、卡片、相框或贴纸中的底纹可作为容器内容进入 Prompt 自由编辑；外围留白缺少独立用户动机时作为固定画布。候选被排除为槽位不代表从 Prompt 消失，仍需根据可编辑或固定决议完成路由。
